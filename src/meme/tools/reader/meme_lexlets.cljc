(ns meme.tools.reader.meme-lexlets
  "Meme lexical scanlets: character predicates, consume helpers,
   trivia consumers, and scanlet builders.
...
(defn atom-scanlet
  ...
  (fn [engine]
    (let [start (pratt/cursor engine)
      ...
      (pratt/set-pos! engine end)
      (let [tok (pratt/make-token! engine token-type start)]
...
(defn delimited-scanlet [node-type token-type close-char close-type]
  (single-char-scanlet token-type (pratt/nud-delimited node-type close-char close-type)))
